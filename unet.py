...
class Unet(object):
    """
    OO implementation of U-Net for Kaggle image segmentation challange
    ...
    def __init__(self, img_rows=512, img_cols=512, batch_size=8):
            ...
            for start in range(0, len(imgnames), self.batch_size):
                ...
                for i in range(start, end): 
                    ...
                    if label_dir is not None:
                        ...
                        mask_arr = img_to_array(mask)/255
    ...
    def model(self):
        ...
        model.compile(optimizer = Adam(lr=1e-4), loss = 'binary_crossentropy', metrics = [self.dice_coef])
        model.summary()

        return model


    def train(self, train_generator, validation_generator, n_epoch=20, train_steps=8, save_history=False):
    ...
    def predict(self, weight_dir, test_generator, test_label_dir):
        ...
        masks_arr_predicted = model.predict_generator(test_generator, steps=2)
        
        for i in range(masks_arr_predicted.shape[0]):
            mask_arr_predicted = masks_arr_predicted[i]
            mask_predicted = array_to_img(mask_arr_predicted)
            test_label_path = os.path.join(test_label_dir, '{}_unet.gif'.format(i))
...
if __name__ == '__main__':
    ...
    unet.train(train_gen, valid_gen, n_epoch=n_epoch, save_history=True)
    unet.plot_history(n_epoch=n_epoch)
    unet.predict('Unet.hdf5', test_gen, test_label_dir)
